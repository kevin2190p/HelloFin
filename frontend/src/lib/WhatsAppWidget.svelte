...
<div class="wa">
  <div class="wa-hdr">
    <div class="back-btn"><svg viewBox="0 0 24 24" width="20" height="20" fill="#fff"><path d="M12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2H7.83l5.58-5.59z"/></svg></div>
    <div class="av">
      {#if avatarUrl}
        <img src={avatarUrl} alt="" class="av-img" />
      {:else}
        <svg viewBox="0 0 212 212" width="36" height="36"><path fill="#DFE5E7" d="M106.251.5C164.653.5 212 47.846 212 106.25S164.653 212 106.25 212C47.846 212 .5 164.654.5 106.25S47.846.5 106.251.5z"/><path fill="#FFF" d="M173.561 171.615a62.767 62.767 0 0 0-2.065-2.955 67.7 67.7 0 0 0-2.608-3.299 70.112 70.112 0 0 0-3.184-3.527 71.097 71.097 0 0 0-5.924-5.47 72.458 72.458 0 0 0-10.204-7.026L138.1 143.1a72.584 72.584 0 0 0-9.95-4.298c-.185-.068-.374-.122-.56-.187l-.122-.044-.356-.122c-.232-.083-.467-.157-.7-.235a38.605 38.605 0 0 0 9.752-25.964c0-21.393-17.407-38.8-38.8-38.8s-38.8 17.407-38.8 38.8a38.6 38.6 0 0 0 9.752 25.964c-.233.078-.468.152-.7.235l-.356.122-.122.044c-.186.065-.375.119-.56.187a72.584 72.584 0 0 0-9.95 4.298l-11.476 6.238a72.381 72.381 0 0 0-10.204 7.026 71.097 71.097 0 0 0-5.924 5.47 70.08 70.08 0 0 0-3.184 3.527 67.7 67.7 0 0 0-2.608 3.299 62.767 62.767 0 0 0-2.065 2.955A56.33 56.33 0 0 0 28 186.074c16.907 16.063 39.59 25.926 64.25 25.926 24.662 0 47.344-9.863 64.252-25.926a56.391 56.391 0 0 0-3.94-14.459z"/></svg>
      {/if}
    </div>
  ...
  <div class="wa-chat" bind:this={chatEl}>
    ...
    {/each}
    {#if messages.length===0}<div class="empty-chat"><p>Select a scenario to begin</p></div>{/if}
  </div>
...
<style>
  ...
  .av{width:36px;height:36px;border-radius:50%;overflow:hidden;flex-shrink:0;background:#ccc}
  ...
  .wa-chat{flex:1;overflow-y:auto;padding:8px 12px;background:#efeae2;background-image:url("data:image/svg+xml,%3Csvg width='300' height='300' xmlns='http://www.w3.org/2000/svg'%3E%3Cdefs%3E%3Cpattern id='p' width='40' height='40' patternUnits='userSpaceOnUse'%3E%3Cpath d='M20 0L20 40M0 20L40 20' stroke='%23d6cfc4' stroke-width='0.3'/%3E%3C/pattern%3E%3C/defs%3E%3Crect fill='url(%23p)' width='300' height='300'/%3E%3C/svg%3E");display:flex;flex-direction:column;gap:3px}
  .wa-chat::-webkit-scrollbar{width:4px}.wa-chat::-webkit-scrollbar-thumb{background:rgba(0,0,0,.15);border-radius:2px}
  ...
  .transcript{font-size:.75rem;color:#667781;font-style:italic;margin:4px 0 2px;padding:4px 6px;background:rgba(0,0,0,.03);border-radius:4px;border-left:2px solid #00a884}
  .meta{display:flex;justify-content:flex-end;align-items:center;gap:3px;margin-top:1px}
  .time{font-size:.65rem;color:#667781}.ticks{display:flex;align-items:center}
  ...
  .v-bar{width:2.5px;background:#8696a0;border-radius:2px}.v-dur{font-size:.68rem;color:#667781;flex-shrink:0}
  ...
  .scan-dots{display:flex;gap:3px}.scan-dots span{width:4px;height:4px;border-radius:50%;background:#0071e3;animation:sp .8s ease-in-out infinite}
